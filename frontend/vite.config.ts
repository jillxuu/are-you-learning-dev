...
import fs from "fs";
import { compilePackageTask, publishMovePackage } from "./src/utils/moveUtils";
...
export default defineConfig(({ mode }) => {
  ...
  if (!AI_API_KEY) {
    console.warn(
      "Warning: VITE_CHATGPT_API_KEY environment variable is not set",
    );
  }

  if (!APTOS_NODE_URL) {
    console.warn(
      "Warning: VITE_APTOS_NODE_URL environment variable is not set, using default devnet",
    );
  }

  if (!APTOS_PRIVATE_KEY) {
    console.warn(
      "Warning: VITE_APTOS_PRIVATE_KEY environment variable is not set",
    );
  }
  ...
  return {
          ...
          server.middlewares.use("/api/openai", async (req, res) => {
            ...
            if (req.method === "POST") {
              try {
                ...
                res.end(
                  JSON.stringify({
                    error: "Failed to proxy OpenAI request: " + errorMessage,
                  }),
                );
              }
            } else {
    ...
    define: {
      "process.env.VITE_CHATGPT_API_KEY": JSON.stringify(
        env.VITE_CHATGPT_API_KEY,
      ),
      "process.env.VITE_APTOS_NODE_URL": JSON.stringify(
