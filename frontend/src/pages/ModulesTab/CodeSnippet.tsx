...
function getPublicFunctionLineNumber(sourceCode: string, functionName: string): number {
...
export function Code({ bytecode }: CodeProps) {
  ...
  useEffect(() => {
    if (codeBoxScrollRef.current) {
      codeBoxScrollRef.current.scrollTop = LINE_HEIGHT_IN_PX * startingLineNumber;
    }
  }, [startingLineNumber]);
  ...
  try {
    ...
    const decodingMethods = [
      ...
      () => {
        ...
        const bytes = Uint8Array.from(atob(cleanSource), c => c.charCodeAt(0));
        return new TextDecoder().decode(bytes);
      },
    ...
    for (const method of decodingMethods) {
      try {
        ...
        if (result.includes('module') || result.includes('script') || result.startsWith('{')) {
    ...
    return "// Error decoding source code: " + (error instanceof Error ? error.message : String(error));
  }
}
