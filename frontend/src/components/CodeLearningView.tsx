...
const MOVE_KEYWORDS = {
  ...
  fun: {
    keyword: "fun",
    description:
      "Declares a function that can be called to execute contract logic.",
    docsUrl: "https://aptos.dev/en/build/smart-contracts/book/functions",
  },
  "#[event]": {
    keyword: "#[event]",
    description:
      "Marks a struct as an event that can be emitted during contract execution.",
    docsUrl:
      "https://aptos.dev/en/build/smart-contracts/book/structs-and-resources",
  },
  public: {
    ...
    docsUrl:
      "https://aptos.dev/en/build/smart-contracts/book/functions#visibility",
  },
  entry: {
    keyword: "entry",
    description:
      "Marks a function as an entry point that can be called directly in a transaction.",
    docsUrl:
      "https://aptos.dev/en/build/smart-contracts/book/functions#entry-modifier",
  },
  "has key": {
    keyword: "has key",
    description:
      "Indicates that a struct can be used as a key in global storage.",
  ...
  "has drop": {
    keyword: "has drop",
    description:
      "Indicates that a struct can be dropped without explicit destruction.",
    docsUrl: "https://aptos.dev/en/build/smart-contracts/book/type-abilities",
  },
} as const;
...
export default function CodeLearningView({ code, onContinue }: Props) {
  ...
  const [selectedType, setSelectedType] = useState<
    "function" | "struct" | "event"
  >("struct");
  const [selectedComponent, setSelectedComponent] = useState<string | null>(
    null,
  );
  const [selectedKeyword, setSelectedKeyword] = useState<MoveKeyword | null>(
    null,
  );
    ...
    if (selectedComponent && !isLoading) {
      requestAnimationFrame(() => {
        ...
        if (element && explanationsContainerRef.current) {
          ...
          const scrollTop =
            container.scrollTop + (elementRect.top - containerRect.top) - 48;
          container.scrollTo({
            top: scrollTop,
            behavior: "smooth",
          });
        ...
        const component = currentExplanationsRef.current.find((exp) => {
          ...
          if (lineContent.includes("fun") || lineContent.includes("struct")) {
            ...
            if (lineContent.includes(`struct ${clickedWord}`)) {
              ...
              if (
                (isEvent && exp.type === "event") ||
                (!isEvent && exp.type === "struct")
              ) {
