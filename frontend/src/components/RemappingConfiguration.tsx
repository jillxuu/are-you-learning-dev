...
export const RemappingConfiguration = ({
    ...
    const fetchEventFields = async () => {
      ...
      try {
        ...
        selectedEvents.forEach((eventFullName) => {
          ...
          if (module) {
            const eventStruct = module.abi.structs.find(
              (s: any) => s.name === eventName && s.is_event,
            );
            if (eventStruct) {
              newEventFields[eventFullName] = eventStruct.fields.map(
                (f: any) => ({
                  name: f.name,
                  type: f.type,
                }),
        ...
        const initialMappings = selectedEvents.map((event) => ({
          eventName: event,
          fields:
            newEventFields[event]?.map((field) => ({
        ...
        setError(
          err instanceof Error ? err.message : "Failed to fetch event fields",
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = () => {
    ...
    const hasEmptyFields = mappings.some((mapping) =>
      mapping.fields.some((field) => !field.sourceField || !field.targetField),
    );
    ...
    const config = {
      events: mappings.reduce(
        (acc, mapping) => {
          acc[mapping.eventName] = {
            event_fields: mapping.fields.reduce(
              (fieldAcc, field) => {
